...
class GoogleCode
  def spider(keywords)
    # Number of hits pr page
    hits_pr_page = 10
    # Fetch the initial page
    initial_page = Nokogiri::HTML(open("http://code.google.com/hosting/search?q=erlang&projectsearch=Search+projects"))
    ...
    if count_string.length == 3
      total_hits = count_string[2]
      # Process the first page
      process_search_page(initial_page)
      # Calculat the number of pages
      total_pages = (total_hits.to_i / hits_pr_page) + 1
      total_pages.times.each {|page|
        start_index = page * 10
        search_page = Nokogiri::HTML(open("http://code.google.com/hosting/search?q=erlang&filter=1&start=#{start_index}"))
        # Process the page
  ...
  def process_search_page(initial_page)
